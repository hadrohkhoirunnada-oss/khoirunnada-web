...
export default function BookingAdminSelect({
  admins,
  selectedAdminId,
  onChange,
}) {
  ...
  const selectedAdmin =
    activeAdmins.find((admin) => admin.id === selectedAdminId) ||
    activeAdmins[0];

  const handleSelect = (adminId) => {
    onChange(adminId);
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative" ref={wrapperRef}>
      ...
      <button
        ...
        className={`mt-3 flex min-h-13 w-full items-center justify-between gap-3 rounded-2xl border bg-black/38 px-4 text-left text-sm font-extrabold text-white shadow-lg shadow-black/20 outline-none backdrop-blur-xl transition ${
          isOpen
            ? "border-amber-300/45 bg-black/48 shadow-[0_0_0_3px_rgba(245,197,66,0.08)]"
            : "border-amber-300/14"
        }`}
      ...
      >
        <span className="min-w-0 flex-1 truncate">
          {selectedAdmin
            ? `${selectedAdmin.label} — ${selectedAdmin.name}`
            : "Pilih admin tujuan"}
        </span>
      ...
      {isOpen ? (
        <div className="absolute left-0 right-0 top-full z-40 mt-2 overflow-hidden rounded-2xl border border-amber-300/18 bg-[#070910]/95 p-1.5 shadow-2xl shadow-black/50 backdrop-blur-2xl">
